        ...
        toProperty: function (client, pElement) {
            ...
            if (tfElements.length > 1) {
                ...
                for (var i = 0; i < tfElements.length; ++i) {
                    var el = tfElements[i];
                    if (el.getAttribute("remote") === "true") {
                        for (var j = 0; j < el.children.length; ++j) {
                            var el2 = el.children[j];
                            switch (el2.tagName) {
                                case "n":
                                    font.typeface[i] = el2.firstChild.data;
                                    break;
                                case "tfc":
                                    font.cssLink.push(el2.firstChild.data);
                                    break;
                                case "tfp":
                                    font.preconnectLink.push(el2.firstChild.data);
                                    break;
                            }
                        }
                    } else {
                        font.typeface[i] = el.firstChild.data;
                    }
                }
            ...
            if (element.getAttribute("bo")) {
                font.bold = true;
            }
            if (element.getAttribute("it")) {
                font.italic = true;
            }
            if (element.getAttribute("un")) {
                font.underline = true;
            }
            if (element.getAttribute("ov")) {
                font.overline = true;
            }
            if (element.getAttribute("lt")) {
                font.lineThrough = true;
            }
            if (font.typeface){
               console.log("FONT " + font.typeface[0], font);
            }
            return font;
        }
    },
