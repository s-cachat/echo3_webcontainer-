...
/**
 * Factory for obtaining XML serial peer implementations for specific property
 * classes )for the web container)
 */
class PropertySerialPeerFactory implements PropertyPeerFactory {
     ...
     */
    private static final Map<ClassLoader, PropertySerialPeerFactory> classLoaderToFactoryMap = new HashMap<>();

    /**
     * Creates or retrieves a <code>SerialPeerFactory</code>.
     *
     * @param classLoader the <code>ClassLoader</code> to use for dynamically
     * loading peer classes
    ...
    public static PropertySerialPeerFactory forClassLoader(ClassLoader classLoader) {
        synchronized (classLoaderToFactoryMap) {
            PropertySerialPeerFactory factory = classLoaderToFactoryMap.get(classLoader);
            if (factory == null) {
