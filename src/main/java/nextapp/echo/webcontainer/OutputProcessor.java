...
class OutputProcessor {
            ...
            if (componentUpdates[i].hasAddedChildren() || componentUpdates[i].hasUpdatedProperties()) {
                ...
                if (addedChildren.length > 0) {
                    ...
                    for (int j = 0; j < addedChildren.length; ++j) {
                        ...
                        indexedComponents.put(parentComponent.visibleIndexOf(addedChild), addedChild);
                    }
    ...
    throws SerialException {
        Document _document = parentElement.getOwnerDocument();
        ...
        Element cElement = _document.createElement("c");
        ...
        if (c.getFocusNextId() != null || c.getFocusPreviousId() != null) {
            Element fElement = _document.createElement("f");
        ...
        if (!c.isEnabled()) {
            Element enElement = _document.createElement("en");
            enElement.appendChild(_document.createTextNode("false"));
            cElement.appendChild(enElement);
        }
        
        // Render locale.
        if (c.getLocale() != null) {
            Element localeElement = _document.createElement("locale");
            localeElement.appendChild(_document.createTextNode(getClientLocaleString(c.getLocale())));
            cElement.appendChild(localeElement);
        }
        
        // Render layout direction.
        if (c.getLayoutDirection() != null) {
            Element dirElement = _document.createElement("dir");
            dirElement.appendChild(_document.createTextNode(c.getLayoutDirection().isLeftToRight() ? "ltr" : "rtl"));
        ...
        while (eventTypeIterator.hasNext()) {
            ...
            Element eElement = _document.createElement("e");
    ...
    throws SerialException {
        Document _document = sElement.getOwnerDocument();
        ...
        while (it.hasNext()) {
            ...
            if (ci.isIndexedProperty(propertyName)) {
                ...
                while (indicesIt.hasNext()) {
                    ...
                    Element pElement = _document.createElement("p");
                    pElement.setAttribute("n", propertyName);
                    // Set property index.
    ...
    throws SerialException {
        ...
        Document _document = ssElement.getOwnerDocument();
        ...
        while (styleNameIterator.hasNext()) {
            ...
            while (componentTypeIterator.hasNext()) {
                ...
                Element sElement = _document.createElement("s");
